...
def main_chart():
    ...
    # Add figure layout
    fig.update_layout(title_text=(f'<span style="font-size: 18px;"> \
        Total Electricity Generated and Carbon Emission {year_chosen[0]} - \
        {year_chosen[1]}</span>'),
        hovermode='x unified',
        plot_bgcolor='#FFFFFF',
        barmode = 'stack',
...
def group_charts(df2,df2_1,df2_2,clk_year,daterange):
    ...
    fig2.update_layout( height=820,
        ...
        title_text=(f'<span style="font-size: 18px; ">Detail Data \
            of year {clk_year}</span> <span style="font-size: 12px; font-style: italic"> \
            (Click on the bar chart above to view different year)</span>'),
        title_x= 0.035,
        hovermode='x unified',
...
def pie_chart(df3,hov_year):
    ...
    fig3.update_layout(title_text=(f'<span style="font-size: 18px; >\
        Electricity Generated by Sources Year {hov_year}</span>'),
        margin = dict(l=0,r=0),
        legend=dict(orientation='h',yanchor='top',y=-0.1,xanchor='left',x=0)
        )
...
def daily_charts(df3,df4,clk_date):
    ...
    fig4.update_layout( height=622,
        plot_bgcolor = '#FFFFFF', paper_bgcolor = '#FFFFFF',
        title_text=(f'<span style="font-size: 18px; ">Detail Data \
            of {sel_date}</span> <span style="font-size: 12px; font-style: italic"> \
            (Click on the bar chart above to view different day)</span>'),
